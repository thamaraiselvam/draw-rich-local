...
def enrich_d2_styles(d2_source: str) -> str:
    ...
    for line in d2_source.splitlines():
        ...
        # Update depth based on braces in the current line
        # Count opening braces
        depth += stripped.count("{")
        
        # Only treat as plain node if:
        # 1. Line contains ":"
        # 2. No arrows (not a connection)
        # 3. No braces in the line itself
        # 4. We are at depth 0 (top-level)
        is_plain_node = (
            ":" in stripped
            and "->" not in stripped
            and "{" not in stripped
        ...
        if is_plain_node:
            ...
            if icon and color:
                ...
                depth += 1  # Account for the opening brace we just added
                # Skip adding the original line since we've wrapped it
        else:
            enriched.append(line)
        
        # Update depth based on closing braces
